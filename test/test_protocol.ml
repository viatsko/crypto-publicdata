...
let%expect_test "subscribe without exchange filter takes the default rate" =
  ...
  [%expect {| (Subscribe (channel tickers) (exchanges ()) (rate 1) (include_index false)) |}]
;;
...
let%expect_test "subscribe rate is clamped low" =
  ...
  [%expect {| (Subscribe (channel tickers) (exchanges ()) (rate 0.1) (include_index false)) |}]
;;

let%expect_test "subscribe rate is clamped high" =
  show_incoming {|{"op":"subscribe","channel":"tickers","rate":60}|};
  [%expect {| (Subscribe (channel tickers) (exchanges ()) (rate 5) (include_index false)) |}]
;;

let%expect_test "subscribe accepts integer rate" =
  show_incoming {|{"op":"subscribe","channel":"tickers","rate":2}|};
  [%expect {| (Subscribe (channel tickers) (exchanges ()) (rate 2) (include_index false)) |}]
;;
...
let%expect_test "includeIndex defaults to false" =
  show_incoming {|{"op":"subscribe","channel":"tickers"}|};
  [%expect
    {| (Subscribe (channel tickers) (exchanges ()) (rate 1) (include_index false)) |}]
;;

let%expect_test "includeIndex=true parses" =
  show_incoming {|{"op":"subscribe","channel":"tickers","includeIndex":true}|};
  [%expect
    {| (Subscribe (channel tickers) (exchanges ()) (rate 1) (include_index true)) |}]
;;

let%expect_test "includeIndex must be a bool" =
  show_incoming {|{"op":"subscribe","channel":"tickers","includeIndex":"yes"}|};
  [%expect {| ERROR: includeIndex must be a boolean |}]
;;
...
let%expect_test "snapshot with data serialises nested exchange→symbol map" =
  ...
  [%expect
    {|
    {
      "type": "snapshot",
      "channel": "tickers",
      "data": { "bybit": { "BTCUSDT": { "l": 100.0 }, "ETHUSDT": { "l": 5.0 } } }
    }
    |}]
;;
