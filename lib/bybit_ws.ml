...
let ws_url : Exchange.t -> Uri.t = function
  ...
  | Hyperliquid | Hyperliquid_spot | Bitget | Coinbase | Kraken ->    failwith "Bybit_ws.ws_url: not a bybit variant"
;;
...
module Partial = struct
  ...
  let apply ~(exchange : Exchange.t) (t : Ticker.t) partial =
    ...
    let funding_rate =
      ...
      | Binance | Binance_spot | Binance_inverse
      | Hyperliquid | Hyperliquid_spot | Bitget | Coinbase | Kraken -> t.funding_rate
    in
...
let partial_of_data ~(exchange : Exchange.t) json =
  ...
  ; funding_rate =
      (match exchange with
       ...
       | Binance | Binance_spot | Binance_inverse
       | Hyperliquid | Hyperliquid_spot | Bitget | Coinbase | Kraken -> None)
