...
let category_string : Exchange.t -> string = function
  ...
  | Binance | Binance_spot | Binance_inverse | Hyperliquid | Hyperliquid_spot | Bitget | Coinbase | Kraken ->    failwith "Bybit_rest.category_string: not a bybit variant"
;;
...
let normalize_open_interest ~exchange ~raw ~mark ~last =
  ...
  | Bybit | Bybit_spot | Binance | Binance_spot | Binance_inverse
  | Hyperliquid | Hyperliquid_spot | Bitget | Coinbase | Kraken -> raw
;;
...
let ticker_of_json ~(exchange : Exchange.t) json =
  ...
  | `Assoc _ ->
    ...
    let funding_rate =
      ...
      | Hyperliquid | Hyperliquid_spot | Bitget | Coinbase | Kraken ->        parse_float (string_field json "fundingRate")
    in
